...
fn walk_files(root: &Path) -> WalkResult {
    ...
    // Route through the shared ignore policy (`.gitignore` + `.tilthignore`,
    // `SKIP_DIRS`) so the fingerprint counts the same files search does.
    // The bespoke recursion this replaced captured files at path-depths 1..=3
    // (depth 0 was the root, max_depth=2 gated recursion two levels in);
    // `max_depth(Some(3))` is the WalkBuilder equivalent.
    let mut builder = crate::search::base_walk_builder(root);
    ...
    for entry in builder.build().flatten() {
        ...
        *result.lang_counts.entry(lang).or_insert(0) += 1;
        ...
        result
            .code_files
            .push((rel.to_string_lossy().to_string(), size));
